Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\DuckDB.Client").Path
	$manifest = ((Get-Content "$moduleRoot\DuckDB.Client.psd1") -join "`n") | Invoke-Expression

	$config = Import-PowerShellDataFile -Path "$global:testroot\..\config.psd1"
	if (-not $config.ExportFunctions) {
