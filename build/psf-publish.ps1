...
if ($LocalRepo) {
	Write-Host "Creating Nuget Package for module: 'DuckDB.Client' at '$(Get-Location)"
	Publish-PSFModule -Path "$($publishDir)\DuckDB.Client" -DestinationPath . -SkipDependenciesCheck
	return
}

# Publish to Gallery
Write-Host "Publishing the DuckDB.Client module to $($Repository)"
$param = @{
	Path                  = "$($publishDir)\DuckDB.Client"
